...
Mat FaceAlignment(Mat& input, Size size) {
	Mat output;
...
void main()
{
	...
	while (getline(infile, line)) {
		...
		Mat alignedFaceImg = FaceAlignment(facelmg, Size(86, 106)); // 얼굴 정렬
		Mat LBPCodes = LBPEncoding(alignedFaceImg, 8, 3); // LBP feature vector 생성
	...
	while (getline(infile, line)) {
		...
		Mat alignedFaceImg = FaceAlignment(facelmg, Size(86, 106)); // 얼굴 정렬
		Mat LBPCodes = LBPEncoding(alignedFaceImg, 8, 3);
